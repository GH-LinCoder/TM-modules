...
async    resolveCurrentSubject() { //calls the imported function & puts values in this. globals
    ...
    // 🔹 Fetch survey assignment ids from the assignment table for a person
...
console.log("Survey call payload:", {
  ...
  type: 'survey'
        ...
        return assignments.map(a => a.survey_header_id); //returns just the ids
      } catch (error) {
  ...
  // 17:40 Dec 25 2025 I don't understand the point of this. I have removed the call to this method
    buildAutomationMap(rows) {  //why bother? Only relevant if an answer is clicked. 
      ...
      for (const row of rows) { //loop through each row of survey data
        ...
        const automation = row.relationship   //check what type of automation and assemble the relavant data for it
          ? {
              type: 'relationship',  // no such thing in survey_view
          ...
          : {
              type: 'task', // no such thing in survey_view
    ...
    async render(panel) {
      ...
      this.panelEl = panel;
      panel.innerHTML = '';
      await this.resolveCurrentSubject(); //calls the imported function & puts values in this. globals
  console.log('resolved subject:',this.subject);
      let surveyIds = [];
      const isMyDash = detectContext(panel);
  
      if (this.subjectType === 'app-human') {
        surveyIds = await this.findAssignedSurveys();//returns just the ids
      ...
      if (surveyIds.length === 0) {
        ...
        return;  // dec 25 13:10 subjectName output as 'unknown'
      }
  
      const surveyData = await this.fetchSurveyData(surveyIds[0]);//read from survey_view
      if (!surveyData.length) return;
  
      this.surveyData = surveyData;

      // 17:40 Dec 25 2025 I don't understand the point of this. It creates a ampa of every answer and automation. But we only need the one that is clicked/
//      this.buildAutomationMap(surveyData); //<--- why here? Only relevant when displaying answers
      ...
      setTimeout(() => {
        ...
        radios.forEach(radio => {
          radio.addEventListener('change', (e) => {
            ...
            //need to call the function to find and run the automations <---------------------------

          // 17:40 Dec 25 2025 I have removed the call to create an automations map so this line will fail  
...
async loadAnswerAutomations(answerId) {
console.log('loadAnswerAutomations for currentAnswer',answerId); //console.log('currentAnswer', currentAnswer, 'taskName',taskName, 'answerId', answerId );
  try {
    const automations = await executeIfPermitted(this.subjectId, 'readTaskAutomations', {
      answer_id: answerId
    });
    //console.log('automations read from db:',automations);
 //executeAutomations(answerId,automations);
