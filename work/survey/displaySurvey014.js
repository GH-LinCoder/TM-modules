...
console.log('displaySurvey (based on editSurvey).js loaded');

///Globals
const state = {
  authUserId : appState.query.defaultManagerId,  
...
export async function render(panel, query = {}) {
  console.log('displaySurveys NEW(from edit) render:');


 state.subject = await resolveSubject();

console.log('resolved subject:',state.subject);

 state.subjectId =state.subject.id;
...
if(state.subject.type==='relations'){panel.innerHTML=''; //??
            return;}
else

try {
    const assignments = await executeIfPermitted(state.subjectId, 'readStudentAssignments', {//from task_assignement_view
      student_id: state.subjectId, 
      type:'survey'
    });
    console.log('assignments',assignments);
 if (!assignments || assignments.length === 0) {
      panel.innerHTML = `<div class="text-gray-500 text-center py-8">No survey assignments found.</div>`;
      return;
    }
...
for (const assignment of assignments) { // 
      const surveys = await readSurveyView(assignment.survey_header_id);
      };
  ...
  initClipboardIntegration(panel);
    ...
    panel.innerHTML = `<div class="text-red-500 text-center py-8">Failed to load survey assignments.</div>`;
    showToast('Failed to load survey assignments', 'error');
  }
