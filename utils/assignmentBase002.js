...
// need to accept a parameter for assignmenttype:  'task' || 'survey'
export class AssignmentBase {
  ...
  getTemplateHTML(title = 'Assignment', instructions = []) {
    ...
    <div class="space-y-2">
              ...
              <select id="dropdown001" data-form="dropdown001" class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                <option value="">Select task or survey</option>
              </select>
            </div>

              <!-- ITEM SELECT -->
              <div class="space-y-2">
                <label for="dropdown002" class="block text-sm font-medium text-gray-700">Select Item</label>
                <select id="dropdown002" data-form="dropdown002" class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                  <option value="">Student or respondent</option>
                </select>
              </div>
            ...
            <div class="space-y-4">
              <!-- SUBJECT SELECT -->
              <div class="space-y-2">
                <label for="dropdown003" class="block text-sm font-medium text-gray-700">Select Subject</label>
  ...
  populateFromClipboard(panel) {
    console.log('populateFromClipboard() this.assignmentType:', this.assignmentType);
...
if (this.assignmentType==='task') {
    ...
    console.log('task confirmed',{dropdown01Items});  
  } else
if (this.assignmentType==='survey') {
  
    dropdown01Items = getClipboardItems({as:'survey'});
    dropdown02Items = getClipboardItems({ as: 'other' });
    //const dropdown03Items  = getClipboardItems({as:'manager'});
    console.log('survey confirmed',{dropdown01Items});  
}
...
//    const items = getClipboardItems({ as: 'item', type: 'tasks' }); // Generic item type
    ...
    if (dropdown001) {
     // console.log('Populating item dropdown with', items.length);
      this.addClipboardItemsToDropdown(dropdown01Items, dropdown001, 'task');  // pointless 'task' ??
    }
    ...
    if (dropdown002) {
     // console.log('Populating subject dropdown with', subjects.length);
      this.addClipboardItemsToDropdown(dropdown02Items, dropdown002, 'student'); // pointless 'student' ?
    }
   ...
   //   console.log('Populating subject dropdown with', subjects.length, 'items');
      this.addClipboardItemsToDropdown(dropdown03Items, dropdown003, 'manager'); // pointless 'manager'?
    }
...
// to check if values are set even though not selected by user
  ...
  addClipboardItemsToDropdown(items, selectElement, type) { // pointless 'type' ?
    ...
    console.log('Added', items.length, 'clipboard items to dropdown');
  }
...
attachDropdownListeners(panel, dropdownNumbers = [1, 2]) {
    console.log('attachDropdownListeners',panel, dropdownNumbers);
    dropdownNumbers.forEach(n => {
      panel.querySelector(`#dropdown00${n}`)?.addEventListener('change', () => {
        this.updateAssignButton(panel);
      });
    });
  }
...
*/
  ...
  async handleAssign(e, panel) {
    ...
    const d001 = dropdown001?.value; //id of task or survey
    const d002 = dropdown002?.value; // id of student or respondent
    const d003 = dropdown003?.value; // id of manager (if there is one)


    if (!d001 || !d002) { //tasks and surveys need at least two appros
    ...
    try {
      ...
      // this shows uuid which are not meaningful
      showToast('Assignment completed successfully!', 'success');
      this.addInformationCard({
        'item': dropdown001?.selectedOptions[0]?.dataset.name +' '+ d001.substring(0, 8) + '...',
        'appro': dropdown002?.selectedOptions[0]?.dataset.name +' '+ d002.substring(0, 8) + '...',
        'manager': d003 ? dropdown003?.selectedOptions[0]?.dataset.name +' '+ d003.substring(0, 8) + '...' :'default',
