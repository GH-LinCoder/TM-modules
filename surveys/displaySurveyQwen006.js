...
// Import your icons
import { icons } from '../registry/iconList.js'; // Replace with actual path
...
class SurveyDisplay {
    ...
    getQuestionHTML(questionRows, progress) {
        ...
        const answersHTML = Array.from(answersMap.values()).map(answer => {
            // Use your centralized icons
            let automationIcons = '';
            answer.automations.forEach(automation => {
                if (automation.type === 'task') {
                    automationIcons += icons.task || '🔧'; // fallback to default if not in icons
                } else if (automation.type === 'relationship') {
                    automationIcons += icons.relationships || '🪪'; // fallback to default if not in icons
                }
            });
