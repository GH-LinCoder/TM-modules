...
class SurveyDisplay {
    ...
    showQuestion() {
        ...
        const uniqueQuestions = [...new Set(this.sortedData.map(row => row.question_number))];
        ...
        const questionRows = this.sortedData.filter(row => row.question_number === currentQuestionNumber);
    ...
    getQuestionHTML(questionRows, progress) {
        ...
        const answersHTML = Array.from(answersMap.values()).map(answer => {
            const hasTask = answer.automations.some(automation => automation.type === 'task');
            const hasRelationship = answer.automations.some(automation => automation.type === 'relationship');
            
            const automationIcons = [];
            if (hasTask) automationIcons.push('🔧');
            if (hasRelationship) automationIcons.push('🪪');
            ...
            return `
                <div class="flex items-start mb-3 p-3 border border-gray-300 rounded-lg bg-white hover:bg-gray-100">
                    ...
                    <label for="answer_${answer.id}" class="flex-1">
                        ...
                        ${automationIcons.length > 0 ? `<span class="ml-2 text-blue-600" title="Triggers automation">${automationIcons.join(' ')}</span>` : ''}
 ...
 <div class="p-6 border-b border-gray-200 justify-center items-center text-center">
                        <h2 class="text-xl font-semibold text-gray-900">${this.sortedData[0].survey_name || 'Survey'}</h2>
